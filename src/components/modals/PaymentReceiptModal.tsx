...
export function, PaymentReceiptModal({
  ...
  useEffect(() => {
    // Reset state when modal closes or payment changes, if(!isOpen || !payment) {
      setFileUrls([]);
      setCurrentIndex(0);
    ...
    // Set loading state immediately when modal opens with attachments, setIsLoadingFiles(true);
    ...
    const loadFiles = async () => {
      try {
        ...
        if (isGDrive && ids.length > 0) {
            ...
            ids.map(async (fileId: string) => {
              try {
                ...
                if (!response.ok) {
                  ...
                  // Check if it's a folder ID error, if(errorData.error?.includes('folder') || errorData.error?.includes('Folder')) {
                ...
                // Check if backend detected a folder and returned multiple files, if(data.isFolder && Array.isArray(data.data)) {
                ...
                // Verify it's not a folder, if(fileData.mimeType === 'application/vnd.google-apps.folder') {
                  console.error(`❌ [Payment Receipts] ID ${fileId} is a folder, not a file`);
          ...
          // Log the final file IDs that will be displayed, if(validFiles.length > 0) {
          ...
          if (validFiles.length === 0 && errorFiles.length > 0) {
            // All files failed - show error message, setFileUrls([{
              fileId: 'error',
        ...
        // If parsing fails, try treating it as a single path string, if(payment.payments_attachment && typeof payment.payments_attachment === "string") {
                    ...
                    {getFileType(currentFile) === 'image' ? (
                      ...
                      <iframe
                        ...
                        onError={() => {
                          ...
                          // Fallback to direct links if proxy fails, if(currentFile?.previewUrl && currentAttachment !== currentFile.previewUrl) {
