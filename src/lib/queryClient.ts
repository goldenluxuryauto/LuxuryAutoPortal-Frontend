...
const computeApiBaseUrl = () => {
  ...
  // In production on other domains, use fallback backend URL, if(import.meta.env.PROD && typeof window !== 'undefined') {
...
export function, buildApiUrl(path: string): string {
  ...
  // This would break the Vite proxy - warn and use relative URL instead, if(import.meta.env.DEV && API_BASE_URL === `http://localhost:5000") {
    console.warn(
...
export function, buildUploadApiUrl(path: string): string {
  const normalizedPath = path.startsWith(`/`) ? path : `/${path}`;
  // When API_BASE_URL is set, all requests go directly - use normal buildApiUrl, if(API_BASE_URL) {
...
export function, getProxiedImageUrl(url: string): string {
  ...
  // If it's a GCS, URL(storage.googleapis.com), proxy it through the backend, if(normalizedUrl.startsWith(`https://storage.googleapis.com/`)) {
  ...
  // For local paths, use buildApiUrl to proxy through backend, if(normalizedUrl.startsWith(`http://") || normalizedUrl.startsWith("https://")) {
...
export function, getEmployeeDocumentUrl(value: string | null | undefined): string | null {
  ...
  // Direct URL, if(trimmed.startsWith(`http")) return trimmed;
  // JSON array format
  try {
...
export const getQueryFn = <T,>({ on401: unauthorizedBehavior }: {
  ...
  async ({ queryKey }) => {
    ...
    if (typeof path !== "string") {
      ...
      // Enhanced logging for mobile and production debugging, if(typeof window !== 'undefined') {
        const isMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent);
        const isProduction = import.meta.env.PROD;

        // Always log in production or on mobile for debugging, if(isProduction || isMobile) {
    ...
    try {
      ...
      timeoutId = setTimeout(() => {
        ...
        // Clear timeout if error occurs, if(timeoutId) {
          clearTimeout(timeoutId);
          timeoutId = null;
        }

        // Enhanced error logging for mobile and production, if(error instanceof Error && error.name === 'AbortError') {
          console.error(`[API] Request, timeout(10s) for ${fullUrl}`);
        }

        // Enhanced error logging for mobile and production, if(typeof window !== 'undefined') {
          const isMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent);
          const isProduction = import.meta.env.PROD;
          ...
          if (isMobile || isProduction) {
            ...
            // Show user-friendly error message in console for debugging, if(isProduction) {
