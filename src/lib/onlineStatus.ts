...
export function, calculateOnlineStatus(
  ...
  const lastLogin = new, Date(lastLoginAt);
    // Check if date is valid, if(isNaN(lastLogin.getTime())) {
  ...
  const minutesAgo = (now.getTime() - lastLogin.getTime()) / (1000 * 60);
    ...
    // Handle negative, values(future dates) - treat as offline, if(minutesAgo < 0) {
      return false;
    }
...
export function, getOnlineStatusBadge(
  ...
  // If lastLogoutAt exists and is more recent, than(or equal to) lastLoginAt, user is offline, if(lastLogoutAt) {
    try {
    ...
    if (lastLoginAt) {
      ...
      // If logout is more recent or equal to login, user is offline, if(logoutTime >= loginTime) {
        return {
          text: "Offline",
  ...
  // Step, 2: If no login time, show as never logged in, if(!lastLoginAt) {
    return {
